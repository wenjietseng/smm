...
public class Move : MonoBehaviour
{
	...
	// Write the position and rotation 5 times per second
        if (Time.time - currentTime > 0.2f) {
            System.IO.File.AppendAllText(GameSettings.place_file, "Time:" + Time.time + " Eyes position:" + eyes.GetComponent<Transform>().position +
                                                                  " Camera rig position:" + GetComponent<Transform>().position + " Eyes rotation:" +
                                                                  eyes.GetComponent<Transform>().eulerAngles.ToString() + "\r\n");
            currentTime = Time.time;
        }
    }
